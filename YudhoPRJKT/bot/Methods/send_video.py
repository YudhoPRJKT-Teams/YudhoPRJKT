...
async def send_video(chat_id: Union[str, int], video: str, caption: str, parse_mode: str, show_caption_above_media: Optional[bool] = False, has_spoiler: Optional[bool] = False, support_streaming: Optional[bool] = True, disable_notification: Optional[bool] = False, protect_content: Optional[bool] = False, reply_chat: Optional[Union[bool, str]] = True, reply_markup: Optional[str] = None) -> DataBots:
  try:
    ...
    f.add_field('support_streaming', str(support_streaming).lower())
    f.add_field('protect_content', str(protect_content).lower())
    # Check Input video
    if video.startswith('http://') or video.startswith('https://'):
      f.add_field('video', video)
    else:
      f.add_field('video', open(video, 'rb').read(), filename=video)
    ...
    async with ClientSession() as client:
      async with client.post(f"{AuthManager.ReadConfig().get('api')}/sendVideo", data=f) as session:
    ...
    CreateLog.Error(f"Error while use methods: sendVideo", str(e))
    return DataBots()